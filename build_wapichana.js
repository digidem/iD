...
function generateCategories() {
    var categories = {};
    glob.sync(__dirname + '/data/presets_wapichana/categories/*.json').forEach(function(file) {
        var field = read(file),
...
function generateFields() {
    var fields = {};
    glob.sync(__dirname + '/data/presets_wapichana/fields/**/*.json').forEach(function(file) {
        var field = read(file),
            id = file.match(/presets_wapichana\/fields\/([^.]*)\.json/)[1];
...
function generatePresets() {
    var presets = {};
    glob.sync(__dirname + '/data/presets_wapichana/presets/**/*.json').forEach(function(file) {
        var preset = read(file),
            id = file.match(/presets_wapichana\/presets\/([^.]*)\.json/)[1];
