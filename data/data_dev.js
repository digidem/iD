iD.data = {
    load: function(path, callback) {
        if (!callback) {
            callback = path;
            path = '';
        }
        iD.util.asyncMap([
            path + 'data/deprecated.json',
            path + 'data/discarded.json',
            path + 'data/imagery.json',
            path + 'data/wikipedia.json',
            path + 'data/presets_wapichana/presets.json',
            path + 'data/presets_wapichana/defaults.json',
            path + 'data/presets_wapichana/categories.json',
            path + 'data/presets_wapichana/fields.json',
            path + 'data/imperial.json',
            path + 'data/feature-icons.json',
            path + 'data/operations-sprite.json',
            path + 'data/locales.json',
            path + 'dist/locales/en.json'
            ], d3.json, function (err, data) {

            iD.data = {
                deprecated: data[0],
                discarded: data[1],
                imagery: data[2],
                wikipedia: data[3],
                presets: {
                    presets: data[4],
                    defaults: data[5],
                    categories: data[6],
                    fields: data[7]
                },
                imperial: data[8],
                featureIcons: data[9],
                operations: data[10],
                locales: data[11],
                en: data[12]
            };

            callback();
        });
    }
};
